...
import com.cloud.utils.nio.NettyClient;
 ...
 *
 **/
public class ModifiedAgent implements HandlerFactory, IAgentControl, AgentStatusUpdater {
    protected static Logger s_logger = Logger.getLogger(ModifiedAgent.class);
    ...
    Timer _timer = new Timer("ModifiedAgent Timer");
    Timer certTimer;
    Timer hostLBTimer;
    ...
    protected String getConnectionName() {
        return "ModifiedAgent-" + _resource.getName();
    }
    ...
    public ModifiedAgent(final IAgentShell shell) {
        _shell = shell;
        client = null;
    ...
    public ModifiedAgent(final IAgentShell shell, final int localAgentId, final ServerResource resource) throws ConfigurationException {
        _shell = shell;
        _resource = resource;
        client = null;
        ...
        client = new NettyClient("Agent" + _resource.getName(), host, _shell.getPort(), _shell.getWorkers(), this);
        ...
        s_logger.info("ModifiedAgent [id = " + (_id != null ? _id : "new") + " : type = " + getResourceName() + " : zone = " + _shell.getZone() + " : pod = " + _shell.getPod() +
    ...
    public void start() {
        ...
        try {
            ...
            throw new CloudRuntimeException("Unable to start agent", e);
        }

    }

    @Override
    public void handleConnect(boolean result) {
        if (!result) {
            final String host = _shell.getNextHost();
    ...
    public void scheduleWatch(final Request request, final long delay, final long period) {
        synchronized (_watchList) {
            if (s_logger.isDebugEnabled()) {
                s_logger.debug("Adding a watch list");
            }
            final WatchTask task = new WatchTask(request, this);
    ...
    public synchronized void lockStartupTask(final NettyClient client) {
        _startup = new StartupTask(client);
    ...
    @Override
    public Task create(final Task.Type type, final byte[] data) {
        return new ServerHandler(type, data);
    }
    ...
    public void processStartupAnswer(final Answer answer, final Response response) {
        ...
        scheduleWatch(response, _pingInterval, _pingInterval);
            ...
            for (int i = 0; i < cmds.length; i++) {
                ...
                try {
                    ...
                    if (cmd instanceof CronCommand) {
                        ...
                        scheduleWatch(request, (long)watch.getInterval() * 1000, watch.getInterval() * 1000);
    ...
    public void processResponse(final Response response) {
        ...
        if (answer instanceof StartupAnswer) {
            processStartupAnswer(answer, response);
    ...
    protected class ShutdownThread extends Thread {
        ModifiedAgent _agent;

        public ShutdownThread(final ModifiedAgent agent) {
    ...
    public class WatchTask extends ManagedContextTimerTask {
        ...
        protected ModifiedAgent _agent;

        public WatchTask(final Request request, final ModifiedAgent agent) {
            super();
            _request = request;
            _agent = agent;
        }
        ...
        protected void runInContext() {
            ...
            try {
                if (_request instanceof Response) {
                    _ugentTaskPool.submit(new ServerHandler(Task.Type.OTHER, _request));
                } else {
                    client.schedule(new ServerHandler(Task.Type.OTHER, _request));
                }
            } catch (final ClosedChannelException e) {
    ...
    public class StartupTask extends ManagedContextTimerTask {
        ...
        public StartupTask(final NettyClient client) {
    ...
    public class ServerHandler extends Task {
        public ServerHandler(final Task.Type type, final byte[] data) {
            super(type, null, data);
        }

        public ServerHandler(final Task.Type type, final Request req) {
            super(type, null, req);
        }
    ...
    public class PostCertificateRenewalTask extends ManagedContextTimerTask {
        ...
        private ModifiedAgent agent;

        public PostCertificateRenewalTask(final ModifiedAgent agent) {
            this.agent = agent;
        }
    ...
    public class PreferredHostCheckerTask extends ManagedContextTimerTask {
        ...
        protected void runInContext() {
            try {
                ...
                if (preferredHost != null && !preferredHost.equals(connectedHost)) {

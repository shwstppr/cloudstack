...
public class Upgrade42010to42100 extends DbUpgradeAbstractImpl implements DbUpgrade, DbUpgradeSystemVmTemplate {
    ...
    protected void migrateConfigurationScopeToBitmask(Connection conn) {
        String scopeDataType = DbUpgradeUtils.getTableColumnType(conn, "cloud", "scope");
        if (!"varchar(255)".equals(scopeDataType)) {
            return;
        }
        DbUpgradeUtils.addTableColumnIfNotExist(conn, "cloud", "new_scope", "BIG_INT");
        migrateExistingConfigurationScopeValues(conn);
        DbUpgradeUtils.dropTableColumnsIfExist(conn, "cloud", List.of("scope"));
        DbUpgradeUtils.changeTableColumnIfNotExist(conn, "cloud", "new_scope", "scope", "BIG_INT 'Bitmask for scope(s) of this parameter'");
    }
    ...
    protected void migrateExistingConfigurationScopeValues(Connection conn) {
        ...
        try (PreparedStatement pstmt = txn.prepareAutoCloseStatement(sql.toString())) {
            ...
            throw new CloudRuntimeException("Failed to migrate existing configuration scope values to bitmask due to: %s", e);
        }
    }
}
